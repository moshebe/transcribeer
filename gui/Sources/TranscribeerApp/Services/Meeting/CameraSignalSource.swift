...
/// Observes camera activation status changes across video devices.
///
/// Does NOT capture video — only reads the `kCMIODevicePropertyDeviceIsRunningSomewhere`
/// flag on each device, which flips when any process opens the camera.
...
/// Also listens for device hot-plug events and installs listeners on newly
/// attached cameras / removes listeners from detached ones.
final class CoreMediaIOSignalSource: CameraSignalSource, @unchecked Sendable {
    private let listenerQueue = DispatchQueue(label: "com.transcribeer.camera-listener")
    ...
    init() {
        ...
        listenerQueue.sync {
            ...
            // Emit initial state if already running.
    ...
    deinit {
        ...
        if let block = systemListenerBlock {
            ...
            CMIOObjectRemovePropertyListenerBlock(
                CMIOObjectID(kCMIOObjectSystemObject), &address, listenerQueue, block,
            )
        }
        continuation?.finish()
    }
    ...
    private static func deviceRunningAddress() -> CMIOObjectPropertyAddress {
    ...
    private static func devicesListAddress() -> CMIOObjectPropertyAddress {
        CMIOObjectPropertyAddress(
    ...
    private static func videoDeviceIDs() -> [CMIOObjectID] {
        var address = devicesListAddress()
    ...
    private static func hasInputStreams(_ deviceID: CMIOObjectID) -> Bool {
